 ...
 */
export function meetsLanguageRequirement(level: string | undefined, target: string): boolean {
 ...
 */
export function getLanguageLevelDistance(level: string | undefined, target: string): number {
...
export function getLanguageLevelRecommendation(
  ...
  return `Your ${current} level is ${distance} level(s) below ${target}. Estimated time to reach target: ${estimatedMonths}-${estimatedMonths + 3} months with regular study.`;
}
 ...
 */
export function normalizeLanguageLevel(level: string | undefined): string | null {
 ...
 */
export function calculateSalaryMatch(offered: number, required: number): number {
 ...
 * @returns Percentage (0-100+) of requirement met
 */
export function calculateExperienceMatch(years: number, required: number): number {
  if (required === 0) return 100;
 ...
 */
export function normalizeSalary(salary: number | string, period: "year" | "month" = "year"): number {
  let amount: number;
 ...
 */
export function fuzzyMatchField(input: string | undefined, target: string): number {
...
export function isItField(field: string | undefined): boolean {
  ...
  const itKeywords = [
    "software", "developer", "programmer", "engineer", "it", "computer",
    "data", "analyst", "scientist", "web", "mobile", "frontend", "backend",
    "fullstack", "devops", "cloud", "cybersecurity", "security", "network",
    "database", "system", "admin", "architect", "tech", "digital", "ai",
...
export function isHealthcareField(field: string | undefined): boolean {
  ...
  const healthcareKeywords = [
    "nurse", "nursing", "doctor", "physician", "medical", "health",
    "therapist", "therapy", "care", "clinical", "hospital", "patient",
    "medicine", "surgery", "dental", "pharmacy", "pharmacist"
  ];
...
export function isStemField(field: string | undefined): boolean {
  ...
  const stemKeywords = [
    "engineering", "engineer", "science", "mathematics", "math", "physics",
    "chemistry", "biology", "technology", "research", "data", "statistics",
    "computer", "software", "electrical", "mechanical", "civil", "chemical"
  ];
...
export function getFieldGuidance(field: string | undefined): string[] {
  ...
  if (isItField(field)) {
    guidance.push("💻 IT field: Germany has high demand for tech professionals. Consider IT Specialist visa if you lack formal degree.");
  }
  
  if (isHealthcareField(field)) {
    guidance.push("🏥 Healthcare field: You'll need professional recognition (Anerkennung) and often B2-C1 German proficiency.");
  }
  
  if (isStemField(field)) {
    guidance.push("🔬 STEM field: Strong demand in Germany. You may qualify for lower Blue Card salary threshold (€45,600).");
  }
