// supabase/functions/visa-recommendation/lib/applicationMetadata.ts (NEW FILE)
...
/**
 * Enhanced visa recommendation response with application metadata
 */
...
/**
 * Main function to get all application metadata
 */
export async function getApplicationMetadata(
    ...
    const [
        ...
        fetchCityPortals(supabase),
        fetchRequiredDocuments(visaCode, supabase),
        fetchAdditionalQuestions(visaCode, input, supabase),
        fetchPreFilledFields(visaCode, input, supabase),
    ]);
...
async function fetchAdditionalQuestions(
    ...
    const filteredQuestions = data.filter((q) => {
        ...
        const dependentFieldValue = (input as any)[
            snakeToCamel(q.depends_on_field)
        ];
...
async function fetchPreFilledFields(
    ...
    for (const mapping of data) {
        const sourceValue =
            (input as any)[snakeToCamel(mapping.source_field_key)];
...
function applyFormatTransformation(
    ...
    if (outputFormat === "international" && value) {
        // Simple formatting - add + if not present
 ...
 */
function applyCalculateTransformation(
    value: any,
    rule: any,
): any {
 ...
 */
function applyLookupTransformation(
    value: any,
    rule: any,
): any {
...
function getAppointmentUrl(
    ...
    // Map cities to their appointment URLs
