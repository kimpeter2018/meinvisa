...
function readableLabel(key: string): string {
  ...
  if (cleanKey.match(/german_level_([a-c][1-2])\+?/i)) {
    const match = cleanKey.match(/([a-c][1-2])\+?/i);
    const level = match && match[1] ? match[1].toUpperCase() : undefined;
...
function generateNotes(candidate: VisaCandidate): string[] {
  ...
  if (nationality && meta.canApplyInCountry) {
    ...
    if (canApplyInCountry) {
      notes.push(
        "✈️ Your nationality allows visa-free entry - you can apply for residence permit after arriving in Germany.",
      );
    }
  }
  ...
  if (missing.some((m) => m.key.includes("german_level"))) {
    notes.push(
      "🗣️ German language improvement will strengthen your application. Consider starting lessons now.",
    );
  }
  ...
  if (
    missing.some((m) =>
      m.key.includes("degree") || m.key.includes("anerkennung")
    )
  ) {
    if (context?.hasDegree && !context?.hasAnerkennung) {
      notes.push(
        "🎓 Get your degree recognized through Anabin database or ZAB (Central Office for Foreign Education).",
      );
    }
  }
  ...
  if (missing.some((m) => m.key.includes("job_offer"))) {
    notes.push(
      "💼 Focus on securing a job offer. Use platforms: LinkedIn, XING, Make it in Germany, StepStone.",
    );
  }
  ...
  if (missing.some((m) => m.key.includes("funds"))) {
    if (category === "education") {
      notes.push(
        "💰 Open a blocked account (Sperrkonto) with approximately €11,904/year. Providers: Fintiba, Expatrio, Coracle.",
      );
    } else {
      notes.push(
        "💰 Prepare proof of sufficient funds (bank statements, sponsorship letter, or income proof).",
      );
    }
  }
  ...
  if (missing.some((m) => m.key.includes("insurance"))) {
    notes.push(
      "🏥 Arrange health insurance before application. Options: Public (TK, AOK) or private insurance.",
    );
  }
  ...
  if (hasFamily && category !== "personal") {
    notes.push(
      "👨‍👩‍👧‍👦 Since you have family in Germany, family reunion visa might also be an option.",
    );
  }
  ...
  if (category === "specialized" && meta.code === "researcher") {
    notes.push(
      "🔬 Contact German research institutions directly. Resources: EURAXESS, DAAD, Alexander von Humboldt Foundation.",
    );
  }
  ...
  if (category === "education") {
    notes.push(
      "🎯 After graduation, you'll be eligible for an 18-month job-seeker permit to find employment in your field.",
    );
  }
  ...
  if (category === "work" && context?.hasJobOffer) {
    notes.push(
      "⏱️ Work visa processing typically takes 6-12 weeks. Start application as soon as you have all documents.",
    );
  }
