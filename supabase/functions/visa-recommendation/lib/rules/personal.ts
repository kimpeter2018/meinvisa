 ...
 */
export function evaluatePersonalPath(
  ...
  const viableCandidates = allCandidates.filter((c) => {
    ...
    if (c.category === "work" && c.score >= 25 && input.hasFamilyInGermany) {
      return true;
    }
...
function buildPersonalNotes(
  ...
  if (topCandidate.code === "family_reunion" || input.hasFamilyInGermany) {
    notes.push(
      "👨‍👩‍👧‍👦 Family reunion visa allows you to join your family member who has legal residence in Germany.",
    );

    // Language requirements vary by family member's status
    notes.push(
      "🗣️ Language requirement: Usually A1 German, but exceptions exist if family member has Blue Card, permanent residence, or is EU citizen.",
    );

    // Spouse-specific notes
    if (
      input.relationship?.toLowerCase().includes("spouse") ||
      input.relationship?.toLowerCase().includes("partner")
    ) {
      notes.push(
        "💑 Spouses of German citizens or permanent residents have easier requirements.",
      );
      notes.push(
        "💼 You'll be eligible for unrestricted work permit immediately upon arrival.",
      );
      notes.push(
        "🏡 After 3 years, you can apply for permanent residence (2 years if you have B1 German).",
      );

      // Children joining (NEW)
      if (input.hasChildren) {
        notes.push(
          `👨‍👩‍👧‍👦 You indicated having children. They can join you in family reunion.`,
        );
        if (input.childrenCount) {
          notes.push(`   • Number of children: ${input.childrenCount}`);
        }
        if (input.childrenAges) {
          notes.push(`   • Ages: ${input.childrenAges}`);
          // Check for minors
          const ages = input.childrenAges.split(",").map((a) =>
            parseInt(a.trim())
          ).filter((a) => !isNaN(a));
          const minors = ages.filter((age) => age < 18);
          if (minors.length > 0) {
            notes.push(
              `   • ${minors.length} child(ren) under 18 - no language requirement`,
            );
          }
          const adults = ages.filter((age) => age >= 18);
          if (adults.length > 0) {
            notes.push(
              `   • ${adults.length} adult child(ren) - may have more stringent requirements`,
            );
          }
        }
      }
      ...
      if (!input.relationshipProof) {
        notes.push(
          "📄 You'll need relationship proof: marriage certificate, apostilled & translated",
        );
      }
    }

    // Children-specific notes
    if (input.relationship?.toLowerCase().includes("child")) {
      notes.push(
        "👶 Children under 16 don't need German language certification.",
      );
      notes.push(
        "🎒 School enrollment is mandatory for children aged 6-16 (compulsory education).",
      );

      if (input.age && input.age >= 18) {
        notes.push(
          "⚠️ Adult children (18+) have more restrictive reunion requirements",
        );
        notes.push("   • Usually requires proof of exceptional hardship");
        notes.push("   • Or student visa may be better option");
      }
    }
    // Parent reunification
    if (input.personalRoute?.toLowerCase().includes("parent")) {
      notes.push(
        "👴 Parent reunification is more restrictive - requires proof that sponsor can support you financially.",
      );
      notes.push(
        "💰 Sponsor needs sufficient income (typically ~€1,500-2,000/month + additional per family member).",
      );
    }
    ...
    if (!input.proofFunds) {
      notes.push(
        "💰 Your family member (sponsor) must prove sufficient income/funds to support you without social benefits.",
      );
    }
    ...
    if (!input.hasAccommodation) {
      notes.push(
        "🏠 Adequate housing required: minimum space standards apply (varies by city, typically 12-15 sqm per person).",
      );
    }

    notes.push(
      "📄 Required documents: Marriage certificate, birth certificates, family member's residence permit, housing proof.",
    );
  }

  // Au Pair specific guidance
  if (
    topCandidate.code === "au_pair" ||
    input.personalRoute?.toLowerCase().includes("au pair")
  ) {
    notes.push(
      "🏡 Au pair visa: Live with German host family, provide childcare (~30 hrs/week), receive pocket money (~€280/month) + accommodation.",
    );
    ...
    if (input.age) {
      if (input.age >= 18 && input.age <= 26) {
        notes.push(
          `✓ Your age (${input.age}) fits the au pair requirement (18-26 years).`,
        );
      } else if (input.age < 18) {
        notes.push(
          `❌ You must be at least 18 years old for au pair visa (currently ${input.age}).`,
        );
      } else if (input.age > 26) {
        notes.push(
          `⚠️ Au pair programs typically require age 18-26 (you're ${input.age}). Some families may accept slightly older applicants.`,
        );
      }
    } else {
    ...
    notes.push(
      "🎓 You must attend German language course (minimum 6 hours/week) - included in program.",
    );
    notes.push(
      "⏱️ Au pair stay is limited to 12 months (some extend to 24 months).",
    );
    notes.push(
      "💼 After au pair year, you can switch to work/study visa if you meet requirements.",
    );
    notes.push(
      "🔍 Find families through: Au Pair World, AuPair.com, Cultural Care, AIFS",
    );
  }

  // Volunteer Service (FSJ/BFD) guidance
  if (
    topCandidate.code === "volunteer_service" ||
    input.personalRoute?.toLowerCase().includes("volunteer")
  ) {
    notes.push(
      "🤝 Volunteer visa (FSJ/BFD/ESC): Work with social, ecological, or cultural organizations.",
    );
    notes.push(
      "💰 You receive: Housing + meals + pocket money (~€150-400/month) + health insurance.",
    );
    ...
    notes.push(
      "📚 Program includes: German language classes, educational seminars, cultural integration.",
    );
    notes.push(
      "🎓 Great pathway to work/study visa: Many volunteers continue with education or employment in Germany.",
    );
    notes.push(
      "🌍 Programs: FSJ (Freiwilliges Soziales Jahr), BFD (Bundesfreiwilligendienst), European Solidarity Corps",
    );
    notes.push(
      "🔍 Find organizations: ijgd.de, pro-fsj.de, bundes-freiwilligendienst.de",
    );
  }
  ...
  if (input.nationality) {
    ...
    if (isEligible && input.age && input.age >= 18 && input.age <= 30) {
      ...
      if (!hasWHVisa) {
        notes.push(
          `🎒 As a ${input.nationality} national aged ${input.age}, you're eligible for Working Holiday Visa (18-30 years)!`,
        );
        notes.push(
          "🌍 Working Holiday: 12 months stay + unlimited work permission + travel freedom. Great way to explore Germany.",
        );
      }
    }
  }
  ...
  if (input.age) {
    if (input.age < 18) {
      notes.push(
        "👶 As a minor, you'll need parental consent and guardian arrangements in Germany.",
      );
    } else if (input.age > 60 && topCandidate.code === "family_reunion") {
      notes.push(
        "👴 Senior applicants: Consider health insurance requirements carefully (can be expensive for private insurance).",
      );
    }
  }
  ...
  if (!germanLevel || ["A1", "A2"].includes(germanLevel)) {
    ...
    notes.push(
      "📖 Free resources: DW Learn German, Goethe Institut free courses, VHS Volkshochschule (~€100-300 for full course)",
    );
  }
  ...
  if (topCandidate.category === "personal") {
    notes.push(
      "🎓 After arrival, you're eligible for Integration Course (Integrationskurs): 600-900 hours German + 100 hours orientation.",
    );
    notes.push(
      "💡 Integration course costs: €2.29/hour (subsidized) or free if receiving social benefits.",
    );
  }
  ...
  if (topCandidate.category === "personal") {
    if (topCandidate.code === "family_reunion") {
      notes.push(
        "💼 Work permission: Usually unlimited work rights if family member has work permit/Blue Card/permanent residence.",
      );
    } else if (topCandidate.code === "au_pair") {
      notes.push(
        "💼 Work restriction: Only au pair work allowed during program. Can work freely after switching visa.",
      );
    } else if (topCandidate.code === "volunteer_service") {
      notes.push(
        "💼 Work restriction: Only volunteer position allowed. Can apply for work visa after completion.",
      );
    }
  }
  ...
  if (topCandidate.category === "personal") {
    ...
    if (topCandidate.code === "family_reunion") {
      notes.push(
        "   → 3 years with residence permit + B1 German = eligible for permanent residence",
      );
      notes.push("   → 2 years if you have B1 German at time of application");
    } else {
      notes.push(
        "   → Complete personal visa program → Switch to work/education visa → 2-4 years → Permanent residence",
      );
    }
  }
  ...
  if (!input.hasInsurance) {
    ...
    notes.push(
      "   • Public insurance: ~€110/month (students/au pair) or ~€200-400/month (employed)",
    );
    notes.push(
      "   • Private insurance: Varies by age/health (consider carefully for long-term stay)",
    );
  }
  ...
  if (topCandidate.category === "personal") {
    ...
    notes.push(
      "   • Connect with expat communities while building local friendships",
    );
  }
...
function buildPersonalFallback(
  ...
  else if (input.personalRoute?.toLowerCase().includes("au pair")) {
    pathType = "au pair";
  } else if (input.personalRoute?.toLowerCase().includes("volunteer")) {
    pathType = "volunteer";
  }

  notes.push(
    `🎯 You're pursuing a ${pathType} visa pathway. Here's what you need:`,
  );
  ...
  if (pathType === "family reunion") {
    if (!input.hasFamilyInGermany) {
      notes.push(
        "👨‍👩‍👧‍👦 Step 1: Confirm your family member has legal residence in Germany (work permit, Blue Card, permanent residence, or citizenship).",
      );
    }

    notes.push(
      "🗣️ Step 2: Achieve A1 German level (usually required, exceptions for Blue Card holders' families).",
    );
    ...
    notes.push(
      "💰 Step 4: Ensure family member earns enough to support you (~€1,500-2,000/month + more for each family member).",
    );
  } // Au Pair path guidance
  else if (pathType === "au pair") {
    notes.push("🔍 Step 1: Find a host family through:");
    notes.push(
      "   • AuPairWorld.com, AuPair.com, Cultural Care, AIFS, GreatAuPair",
    );
    ...
    notes.push(
      "📝 Step 4: Sign au pair contract with host family (standard template available).",
    );
    notes.push(
      "🏥 Step 5: Arrange health insurance (host family usually helps with this).",
    );
    ...
    notes.push(
      "📝 Step 2: Apply to volunteer positions (social services, ecological projects, cultural institutions).",
    );
    ...
    notes.push(
      "🤝 Step 4: Organization provides: hosting agreement, accommodation, health insurance, stipend.",
    );
  } // Generic personal visa guidance
  else {
    notes.push("📋 Step 1: Clarify your specific personal/family situation.");
    notes.push(
      "💼 Step 2: Check if you have family connections, au pair interest, or volunteer opportunities.",
    );
    notes.push(
      "🗣️ Step 3: Start learning German (at least A1 level recommended).",
    );
  }
