 ...
 */
export function evaluateWorkPath(input: VisaQuestionnaireInput): VisaRecommendation {
...
function buildContextualNotes(
  ...
  if (!input.hasJobOffer && topCandidate.category === "work") {
    notes.push("💡 Tip: Securing a job offer will significantly improve your visa options and processing time.");
  }
  ...
  if (input.hasJobOffer && input.salary) {
    ...
    if (input.salary < shortageThreshold) {
      notes.push(`📊 Your salary (€${input.salary}) is below the Blue Card threshold. Consider negotiating a higher salary or exploring skilled worker visas.`);
    } else if (input.salary >= shortageThreshold && input.salary < blueCardThreshold) {
      notes.push(`📊 Your salary (€${input.salary}) qualifies for Blue Card if your job is in a shortage occupation.`);
    }
  }
  ...
  if (input.germanLevel) {
    ...
    if (["A1", "A2"].includes(level)) {
      notes.push("🗣️ Consider improving your German to B1+ level to unlock more visa options and faster settlement pathways.");
      ...
      if (!hasLanguageCourse) {
        notes.push("💡 A language course visa could be a strategic first step while job searching.");
      }
    }
  } else {
    notes.push("🗣️ German language proficiency (even A2-B1) can significantly improve your visa prospects.");
  }

  // Degree recognition
  if (input.hasDegree && !input.hasAnerkennung) {
    notes.push("🎓 Getting your degree officially recognized (Anerkennung) through Anabin/ZAB will improve your eligibility.");
  }

  // IT specialists special path
  if (input.isItField && !input.hasDegree && (input.experienceYears || 0) >= 3) {
    const hasITVisa = allCandidates.slice(0, 3).some(c => c.code === "it_specialist");
    if (!hasITVisa && input.salary && input.salary >= 50260) {
      notes.push("💻 As an IT professional with 3+ years experience, you may qualify for the IT Specialist visa even without a degree.");
    }
  }

  // Experience considerations
  if (input.experienceYears && input.experienceYears < 2) {
    notes.push("📈 Gaining more work experience (2+ years) will open additional visa pathways.");
  }

  // Cross-category suggestions
  if (topCandidate.category === "work" && topCandidate.score < 60) {
    const educationCandidates = allCandidates.filter(c => c.category === "education" && c.score >= 40);
    if (educationCandidates.length > 0) {
      notes.push("🎯 Consider educational pathways (language course, Ausbildung) as a bridge to work visas.");
    }
  }

  // Application location
  if (input.nationality) {
    const visaFreeCheck = ["United States", "Canada", "Australia", "Japan", "Korea", "United Kingdom"].some(
      country => input.nationality?.toLowerCase().includes(country.toLowerCase())
    );
    
    if (visaFreeCheck && topCandidate.meta?.canApplyInCountry) {
      notes.push("✈️ Your nationality allows visa-free entry. Some visas can be applied for from within Germany.");
    }
  }
 ...
 */
function buildFallbackRecommendation(input: VisaQuestionnaireInput): VisaRecommendation {
  const notes: string[] = [];
  ...
  if (!input.hasDegree && !input.hasVocational) {
    notes.push("🎓 Step 2: Consider getting your qualifications recognized or pursuing education/training in Germany.");
  }
  
  if (!input.germanLevel || ["A1", "A2"].includes(input.germanLevel.toUpperCase())) {
    notes.push("🗣️ Step 3: Improve German language skills to at least B1 level.");
  }

  notes.push("💡 Consider starting with a Job Seeker Visa (if qualified) or Language Course Visa to build your profile.");

  return {
    recommended: {
      code: "preparation_needed",
      name: "Preparation Phase Required",
      summary: "Based on your current profile, you'll need to strengthen certain qualifications before applying for a work visa. See the notes below for specific steps.",
      notes
    },
    notes: [
