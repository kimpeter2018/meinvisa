 ...
 */
export function evaluateEducationPath(
...
function buildEducationNotes(
  ...
  if (!input.admitted && topCandidate.category === "education") {
    notes.push(
      "💡 Secure university admission first. Use Uni-Assist or apply directly to universities.",
    );
    ...
    if (input.previousEducationLevel) {
      ...
      if (prevEd === "high school") {
        ...
        notes.push(
          "📚 Studienkolleg: 1-year program to meet German university entrance requirements",
        );
      }
    }
  }
  ...
  if (input.studyMode?.toLowerCase() === "part-time") {
    notes.push(
      "⚠️ IMPORTANT: Student visas require FULL-TIME enrollment. Part-time study does not qualify!",
    );
  }

  // Studienkolleg guidance (NEW)
  if (
    input.needsStudienkolleg ||
    input.eduLevel?.toLowerCase().includes("preparatory")
  ) {
    notes.push("🏫 Studienkolleg (Preparatory Course):");
    notes.push("   • Duration: 1 year");
    notes.push("   • Ends with Feststellungsprüfung (assessment test)");
    notes.push("   • Qualifies you for German university admission");
    notes.push(
      "   • Course types: T-Kurs (Technical), M-Kurs (Medical), W-Kurs (Economics), etc.",
    );
  }

  // Language requirements for study (ENHANCED)
  if (input.admitted) {
    const germanLevel = input.germanLevel?.toUpperCase();
    const englishLevel = input.englishLevel?.toUpperCase();
    const studyLang = input.studyLanguage?.toLowerCase();
    ...
    if (
      ...
      notes.push(
        "📖 Consider intensive German course (6-12 months to reach B2)",
      );
    } else if (
      studyLang === "english" &&
      ...
      notes.push(
        "🗣️ English-taught programs require B2-C1 English (TOEFL/IELTS)",
      );
    }

    if (studyLang === "english" && (!germanLevel || germanLevel === "A1")) {
      notes.push(
        "💡 Even for English programs, basic German (A2-B1) helps with daily life and job prospects",
      );
    }
  }
  ...
  if (!input.proofFunds && topCandidate.code === "student") {
    notes.push(
      "💰 You'll need a blocked account (Sperrkonto) with approximately €11,904/year (2024 rate) for living expenses.",
    );
  }
  ...
  if (
    input.isAusbildung ||
    (input.hasVocational && topCandidate.code === "ausbildung")
  ) {
    notes.push(
      "🛠️ Ausbildung is a 2-3 year vocational training with salary. Great pathway to work visa afterward!",
    );
  }
  ...
  if (topCandidate.code === "student") {
    const workCandidates = allCandidates.filter((c) =>
      c.category === "work" && c.score >= 25
    );
    if (workCandidates.length > 0) {
      notes.push(
        "🎯 After graduation, you can switch to a work visa. You'll get 18 months to find a job in your field.",
      );
    } else {
      notes.push(
        "🎯 After graduation, you'll be eligible for an 18-month job-seeker residence permit.",
      );
    }
  }
  ...
  if (input.isNursing) {
    notes.push(
      "🏥 Nursing qualifications need recognition (Anerkennung). Contact your state's recognition office early.",
    );
    notes.push(
      "💡 Many hospitals offer integration programs that combine language training + adaptation courses.",
    );
  }
  ...
  if (input.studyField && input.admitted) {
    ...
    const isStem = stemFields.some((field) =>
      input.studyField?.toLowerCase().includes(field)
    );

    if (isStem) {
      notes.push(
        "🔬 STEM graduates have excellent work visa prospects in Germany's shortage occupations.",
      );
    }
  }
  ...
  if (topCandidate.category === "education" && !input.proofFunds) {
    notes.push(
      "📊 Budget planning: Rent €400-800/month, Health insurance €110/month, Living expenses €600-900/month.",
    );
  }
  ...
  if (topCandidate.code === "student") {
    notes.push(
      "💼 Student visa allows 120 full days OR 240 half days of work per year.",
    );
  }
  ...
  if (input.programStart) {
    ...
    const monthsUntilStart = Math.floor(
      (startDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24 * 30),
    );

    if (monthsUntilStart < 3 && monthsUntilStart > 0) {
      notes.push(
        `⏰ Your program starts in ${monthsUntilStart} months. Apply for visa NOW - processing takes 6-12 weeks.`,
      );
    } else if (monthsUntilStart >= 3) {
      notes.push(
        `📅 You have ${monthsUntilStart} months until program start. You can apply for visa 3 months before arrival.`,
      );
    }
  }
  ...
  if (input.nationality) {
    ...
    if (visaFreeCheck) {
      notes.push(
        "✈️ Your nationality allows visa-free entry. You can enter Germany and apply for residence permit from within the country.",
      );
    }
  }
  ...
  if (!input.hasInsurance && topCandidate.category === "education") {
    notes.push(
      "🏥 Health insurance is mandatory. You can use TK, AOK, or other public insurance (~€110/month for students).",
    );
  }
 ...
 */
function buildEducationFallback(
  input: VisaQuestionnaireInput,
): VisaRecommendation {
  const notes: string[] = [];
  ...
  if (!input.admitted && !input.isLanguageCourse && !input.isAusbildung) {
    notes.push(
      "🎓 Step 1: Get admission to a German university or enroll in a language course.",
    );
    notes.push(
      "📚 Resources: Uni-Assist (uni-assist.de), DAAD (daad.de), Study-in-Germany (study-in-germany.de)",
    );
  }

  if (!input.proofFunds) {
    notes.push(
      "💰 Step 2: Open a blocked account (Sperrkonto) with ~€11,904 for living expenses.",
    );
  ...
  if (
    ...
    notes.push(
      "🗣️ Step 3: Improve language skills. Most programs require B2-C1 German OR B2-C1 English.",
    );
    notes.push(
      "📖 German courses: Goethe Institut, VHS (Volkshochschule), language schools in your country",
    );
  }

  notes.push(
    "💡 Consider starting with a language course visa (easier requirements) while preparing your main study application.",
  );
